...
import com.wonders.project.workflow.common.exception.ProcessException;
...
public class FlowUtil {
	...
	* @throws 
	*/
	public static List<Map<String,Object>> getConfigUser(String processName, String stepName,String typeId){
		String sql = "select 'ST/' || username loginName,userfullname userName "
				+ " ,parent_dept deptId from t_flowsetp_config where processname=?" +
				"and stepname = ? and typeid = ? order by memo ";
		Object[] params = new Object[]{processName,stepName,typeId};
	...
	public static String getQueryListSql(String whereStr,String taskWhereStr){		
		String sql = 
				 " select main.id,main.MODELID,main.INSTANCEID,t.taskid,main.project_name,main.operator_name,main.flag,main.operate_time " +
				 " from pcl_project_discard main, tasks t" +
				 " where " +
				 " main.MODELID=t.processname and main.INSTANCEID = t.incident" +
				 " and main.removed != 1" +
				 " and t.steplabel = 'Begin'" +
				 " " + whereStr +
				 " order by main.operate_time desc";
	...
	//获取汇总人
	public static Map<String,Object> getSimulatePerson(String pname,String pincident,String step){
		Map<String,Object> map = new HashMap<String,Object>();
		String sql = "select t.username loginName from t_approvedinfo t"
				+ "  where t.process=? and t.incidentno =? and "
				+ " t.stepname =? order by t.upddate desc";
		Object[] params = new Object[]{pname,pincident,step};
		List<Map<String,Object>> list = dbUtil.getJdbcTemplate().queryForList(sql, params);
		if(list != null && list.size() > 0){
			map = list.get(0);
		}
	...
	public static int isExistsProjectId(String projectId){
		int count = 0;
		String sql = "select count(t.id) from pcl_project_discard t "
				+ " where t.project_id = ? and t.removed='0'";
		count = dbUtil.getJdbcTemplate().queryForInt(sql,new Object[]{projectId});
