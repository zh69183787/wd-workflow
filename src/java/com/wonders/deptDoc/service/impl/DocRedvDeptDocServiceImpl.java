 ...
 *
 */
@Service("docRedvDeptDocService")
public class DocRedvDeptDocServiceImpl implements DeptDocService {
	@Autowired
	private DeptDocDao<RedvMainBo> dao;
	
	@Override
	public void addDocs(String pname,String pincident,String taskUserLoginName,String userName,String deptId) {
		// TODO Auto-generated method stub
		RedvMainBo bo = this.dao.getBo("from "+RedvMainBo.class.getName() +""
				+ " t where t.removed='0' and t.activeId = ? and t.processInstanceid = ?", new Object[]{pname,pincident});
		//获取流程中部门的部门 & 人
		...
		for(Map.Entry<String, String> entry : catelogMap.entrySet()){
			...
			docFile.setFileName(bo.getActiveId() + " " + bo.getTitle());
			docFile.setRefId(bo.getId());
			docFile.setFilePath(config_ip+url);
			docFile.setFlag(DeptDocConstants.FILE_RECEIVE_DIRECTIVE);
