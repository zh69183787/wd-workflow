...
$(document).ready(function(){
	...
	$("#selectReportUnit").click(function(){
		selectDeptReport("reportUnitTreeZone");
	});
	
	$("#selectHelpUnit").click(function(){
		selectDeptHelp("helpUnitTreeZone");
	});
/*
...
*/	
	...
	$("#clearreport").click(function(){
		return clearReportUnit();
	});
	
	$("#clearhelp").click(function(){
		return clearHelpUnit();
	});
});

function signLeader(id,callback){
	if($("#"+id).length==0) return false;
	var url = contextpath+"/send-deptContact/signLeader.action";
	//console.log(url);
			...
			if(json){
				if(json.length>0){
					...
					for(i=0;i<json.length;i++){
						str+="<option value='"+json[i].loginName+"#"+json[i].name+"'>"+json[i].name+"</option>";
					}
...
function selectDeptReport(zoneId){
	selectDept(zoneId,"-1");
}

function selectDeptHelp(zoneId){
	selectDept(zoneId,"1");
}

function selectDept(zoneId,typeId){
	var url = contextpath+"/send/organTree/deptTree.jsp?zoneId="+zoneId+"&typeId="+typeId+"&random="+Math.random();
	window.open(url);
}
...
function clearReportUnit(){
	if(confirm("是否清除内发部门？")){
		$("#reportUnitId").attr("value","").trigger('change');
		$("#reportUnit").attr("value","");
	}
	return false;
}

function clearHelpUnit(){
	if(confirm("是否清除协办部门？")){
		$("#helpUnitId").attr("value","").trigger('change');
		$("#helpUnit").attr("value","");
	}
	return false;
}

function selectShld(processName,zoneId){
	var url = contextpath+"/send/organTree/shldTree.jsp?processName="+encodeURI(processName)+"&zoneId="+zoneId+"&random="+Math.random();
	window.open(url);
}

function selectHqld(processName,zoneId){
	var url = contextpath+"/send/organTree/hqldTree.jsp?processName="+encodeURI(processName)+"&zoneId="+zoneId+"&random="+Math.random();
	window.open(url);
}
