...
<%
	...
	if("logout".equals(type)){
		session.invalidate();
		String t = ipPath+path+"/send/login.jsp";
...
<html>
  <head>
  	...
  	$(document).ready(function(){
		...
		$(".choice").click(function(){
			var url = "<%=ipPath%>/workflow/send/login.jsp";
  	...
  	function openForm(stepName,taskId,pinstanceId,incidentNo,processName,modelName,taskUserName,assignedtouser){
		var url = "<%=ipPath%>/workflow/send-tDocSend/toFormPage.action?"
			+"stepName="+encodeURI(stepName)+"&"
			+"taskId="+taskId+"&"
			+"pinstanceId="+pinstanceId+"&"//子流程实例号
			+"incidentNo="+incidentNo+"&"//主流程实例号
			+"processName="+encodeURI(processName)+"&"//子流程类型
			+"modelName="+encodeURI(modelName)+"&"//主流程类型
			+"taskUserName="+taskUserName+"&"//当前操作人工号：ST/...
  ...
  <body>
	...
	<%
		...
		List<Map<String,Object>> list = FlowUtil.getActiveProcess(userInfo);
	...
	<br/>
	<div>
		<input type="button" name="add" id="add" value="添加新发文流程" onclick="gotoUrl('<%=path %>/send-tDocSend/toFormPage.action')"/>
		<input type="button" name="add" id="add" value="添加新办公室发文流程" onclick="gotoUrl('<%=path %>/send-tDocSend/toFormPage.action?ngDept=office')"/>
		<input type="button" name="add" id="add" value="添加新党委发文流程" onclick="gotoUrl('<%=path %>/send-tDocSend/toFormPage.action?sendType=120411')"/>
		<input type="button" name="add" id="add" value="添加新纪委发文流程" onclick="gotoUrl('<%=path %>/send-tDocSend/toFormPage.action?sendType=120444')"/>
	</div>
