...
function handleFunc(obj){
	if($.trim($("[name=suggest]").val())==""){
		if($("[name=choice]:checked").val()=="1"){
			$("[name=suggest]").val("同意。");
		}else if($("[name=choice]:checked").val()=="2"){
			$("[name=suggest]").val("拟同意。");
		}else if($("[name=choice]:checked").val()=="5"){
			alert("请输入备注！");
			$("[name=suggest]").focus();
			return false;
		}
	...
	if(confirm("确定处理完成？")){
		var formOptions1 = {
			...
			callback:null,
			dataType :'json',
			url:contextpath+"/send-issue/leaderDeal.action",
...
$(function(){
	if($("#defaultSign").val()=="退回拟稿部门"){
		...
		$("[name=choice]:eq(4)").attr("checked",true);
	}else if($("#defaultSign").val()=="拟同意"){
		$("[name=choice]:eq(1)").attr("checked",true);
	}else {
	...
	<div class="con">
		...
		<table id="choiceZone1" width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr height="30">
				...
				<td><b style="display:inline">同意</b>（对发文没有意见）</td>
			<tr height="30">
				<td class="td_1">
					<input type="radio" name="choice" value="2" />
				</td>
				<td><b style="display:inline">拟同意</b>（请其他领导签发，请在备注中说明）</td>
			</tr>
	           	<tr height="30">
				<td class="td_1">
