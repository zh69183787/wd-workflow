...
<%@page import="com.wonders.contract.workflow.process.util.ProcessUtil"%>
<%
...
<html>
  ...
  <body>
	...
	<%
		...
		List<Map<String,Object>> list = ProcessUtil.generateReviewQueryList();
	...
	//System.out.println(list.size());
		...
		out.println("<tr class='list-title'><td>主流程</td><td>主流程ID</td><td>自有编号</td><td>合同名称</td><td>状态</td><td>查看</td><td>监控</td></tr>");
		
		if(list!=null && list.size()>0){
		for(int i=0;i<list.size();i++){
			Map<String,Object> map = list.get(i);
			String pname = map.get("process").toString();
			String pincident = map.get("incident").toString();
			String cname = map.get("process").toString();
			String cincident = map.get("incident").toString();
			String steplabel = "";
			...
			out.print("<td>"+StringUtil.getNotNullValueString(map.get("contract_self_num"))+"</td>");
			out.print("<td>"+StringUtil.getNotNullValueString(map.get("contract_name"))+"</td>");
	...
	<a href="<%=path %>/contract-process/review/list.jsp" target="_blank">查询列表</a>
	</div>
  </body>
</html>
