...
<meta charset="utf-8" />
	...
	<script type="text/javascript">
		...
		var setting = {
			async: {
				enable: true,
				url:"<%=path %>/discipline-organTree/getComLeaders.action",
				autoParam:["id"],
				dataFilter: filter
			},
			check: {
		...
		function onCheck(e, treeId, treeNode) {
			...
			var cityObj = $("#comLeaderLoginName");
			cityObj.attr("value", v1.join());
			
			var cityObj = $("#comLeaderName");
			cityObj.attr("value", v2.join());
			
			var cityObj = $("#comLeaderDeptId");
		...
		function initParamValue(){
			if(zoneId!=""){
				...
				//console.log(zone);
				root = $(zone).attr("root");
				comLeaderLoginName = $(zone).attr("comLeaderLoginName");
				comLeaderName = $(zone).attr("comLeaderName");
				comLeaderDeptId = $(zone).attr("comLeaderDeptId");
				...
				$.getJSON(
					...
					function(json){
						if(json){
							json[0].nocheck=true;
							root = json[0].id;
							json[0].icon = "<%=path %>/discipline/organTree/img/users.png";
							zNodes = json;
							initByZNodes();
						}
					}
		...
		function intiButton(){
			$("#choiceOk").click(function(){
				$("input[name='" + comLeaderLoginName + "']:not(:disabled)",window.opener.document).attr("value",$("#comLeaderLoginName").val());
				$("input[name='" + comLeaderName + "']:not(:disabled)",window.opener.document).attr("value",$("#comLeaderName").val());
				$("input[name='" + comLeaderDeptId + "']:not(:disabled)",window.opener.document).attr("value",$("#comLeaderDeptId").val());
...
<form>
	...
	<input id="comLeaderName" type="text" readonly value="" style="width:75%;"/>
	<input id="comLeaderLoginName" type="hidden" readonly value="" style="width:80%;" />
	<input id="comLeaderDeptId" type="hidden" readonly value="" style="width:80%;" />
