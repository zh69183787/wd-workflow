...
opw = window.open(url,name,'height='+iHeight+',width='+iWidth+',top='+iTop+',left='+iLeft);  
}  
/*
 * 显示文件上传页面(流程逐条意见中使用)
 * 使用示例：
...
function drawAttach(fileGroup, fileGroupName, fileGroupId, newGroupId, procType, targetType, fileCntObjId) {
	...
	openwindow(url,'attachOPT',600,200);
...
function initSuggestAttachShow(){
	$(":hidden[name='suggest_attachId']").each(function(i,n){
		...
		if($(n).val()!=""){
			...
			$(p).append($("<img class=\"attach_icon\" title=\"查看意见附件\" style=\"display:inline;\" src=\""+contextpath+"/receive/css/default/images/new_images/fj.gif\"></img>"));
