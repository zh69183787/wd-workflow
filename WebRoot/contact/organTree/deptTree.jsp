...
<link rel="stylesheet" href="<%=path %>/contact/organTree/css/demo.css" type="text/css">
	...
	<script src="<%=path %>/contact/organTree/js/jquery-1.7.2.min.js"></script>
	<script src="<%=path %>/contact/organTree/js/jquery.ztree.all-3.2.min.js"></script>
	...
	<script type="text/javascript">
		...
		var setting = {
			async: {
				enable: true,
				url:"<%=path %>/contact-organTree/getChildNodes.action",
		...
		function getNodePath(){
			$.ajax({
				type: 'POST',
				url: '<%=path %>/contact-organTree/getRelatedNodes.action?random='+Math.random(),
				data:{
						"id":checkNodes
					},
				dataType:'json',
		...
		function intiButton(){
			$("#choiceOk").click(function(){
				...
				$("#"+targetNameId,window.opener.document).attr("value",$("#theNodeName").val());
				self.close();
				return false;
			});
