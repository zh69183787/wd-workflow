...
<head>
<meta charset="utf-8" />
	<link rel="stylesheet" href="<%=path %>/contact/organTree/css/demo.css" type="text/css">
	<link rel="stylesheet" href="<%=path %>/contact/organTree/css/zTreeStyle/zTreeStyle.css" type="text/css">
	<link type="text/css" href="<%=path %>/contact/organTree/css/flick/jquery-ui-1.8.21.custom.css" rel="stylesheet" />
	
	<script type="text/javascript" src="<%=path %>/contact/organTree/js/jquery-1.7.2.min.js"></script>
	<script type="text/javascript" src="<%=path %>/contact/organTree/js/jquery.ztree.all-3.2.min.js"></script>
	<script type="text/javascript" src="<%=path %>/contact/organTree/js/jquery-ui-1.8.21.custom.min.js"></script>
	...
	<script type="text/javascript">
		...
		var setting = {
			async: {
				enable: true,
				url:"<%=path %>/contact-organTree/getDeptUsersOffLeaders.action",
				autoParam:["id"],
				dataFilter: filter
			},
			check: {
		...
		function initRootNodes(){
			if(zNodes==""){
				var url = "<%=path %>/contact-organTree/getNodesInfo.action";
				$.getJSON(
					url,
					{
						id:root,
		...
		function intiButton(){
			$("#choiceOk").click(function(){
				...
				$("input[name='" + targetNameId + "']:not(:disabled)",window.opener.document).attr("value",$("#theNodeName").val());
...
<form>
	...
	<input id="theNodeName" type="text" readonly value="" style="width:75%;"/>
