...
String zoneId = StringUtil.getNotNullValueString(request.getParameter("zoneId"));
...
<script type="text/javascript">
    $(document).ready(function() {
		...
		if($("#ids").val().length>0){
			var ids = $("#ids").val();
			$.each(ids.split(","),function(n,value){
				$(data_table).find("input:checkbox[value='"+value+"']").attr("checked",true);
	        });
		}
		
		$(data_table).find(":checkbox").click(function(){
			var ids = $("#ids").val();
			var id = $(this).val();
			if(ids.startWith(",")){
			...
			if($(this).attr("checked")){
				...
				if(ids_temp.indexOf(id_temp)==-1){
					ids_temp+=id;
				}
				
				while(ids_temp.startWith(",")){
					ids_temp = ids_temp.substring(1,ids_temp.length);
				}
				while(ids_temp.endWith(",")){
					ids_temp = ids_temp.substring(0,ids_temp.length-1);
				}
				
				$("#ids").val(ids_temp);
			}else{
				if(ids_temp.indexOf(id_temp)>=0){

					ids_temp = ids_temp.replace(id_temp,",");
				}
...
<body>
    <div class="main">
        ...
        <div class="filter">
            <div class="query p8">
            	<form action="<%=path %>/contact-deptContact/refCheckList.action" method="post" id="form">
                    ...
                    <input type=hidden name="ids" id="ids" style="width:100%;" value="<%=ids%>"/>
        ...
        <div class="mb10">
            <table width="100%" class="table_1" id="data_table">
                ...
                <tbody>
					...
					<%--ID	MAIN_UNIT	COPY_UNIT	CONTACT_DATE	REPLY_DATE	THEME	INITIATOR_NAME	CREATE_DEPTNAME	STATUS--%>
                    <s:iterator value="#request.page.result" var="item">
                        <tr name="data_tr">
                            <td class="t_c">
                            	<input type="checkbox" name="ref" value="<s:property value="#item[0]"/>"/>
                            </td>
