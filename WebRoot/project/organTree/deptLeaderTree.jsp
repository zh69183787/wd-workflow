...
<meta charset="utf-8" />
	...
	<script type="text/javascript">
		...
		var dealLeaderId = "";
		var dealLeaderName = "";
		var dealLeaderDeptId = "";
		var treeType = ""; 

		var setting = {
			async: {
				enable: true,
				url:"<%=path %>/project-organTree/getDeptLeaders.action",
				autoParam:["id"],
				dataFilter: filter
			},
			check: {
		...
		function initParamValue(){
			if(zoneId!=""){
				...
				//console.log(zone);
				root = $(zone).attr("root");
				dealLeaderId = $(zone).attr("dealLeaderLoginName");
				dealLeaderName = $(zone).attr("dealLeaderName");
				dealLeaderDeptId = $(zone).attr("dealLeaderDeptId");
		...
		function initRootNodes(){
			if(zNodes==""){
				var url = "<%=path %>/project-organTree/getNodesInfo.action";
				$.getJSON(
					url,
					{
						id:root,
					...
					function(json){
						if(json){
							json[0].nocheck=true;
							root = json[0].id;
							json[0].icon = "<%=path %>/project/organTree/img/com.png";
							zNodes = json;
							initByZNodes();
						}
					}
		...
		function intiButton(){
			$("#choiceOk").click(function(){
				$("input[name='" + dealLeaderId + "']:not(:disabled)",window.opener.document).attr("value",$("#dealLeaderLoginName").val());
				$("input[name='" + dealLeaderName + "']:not(:disabled)",window.opener.document).attr("value",$("#dealLeaderName").val());
				$("input[name='" + dealLeaderDeptId + "']:not(:disabled)",window.opener.document).attr("value",$("#dealLeaderDeptId").val());
...
<form>
	...
	<input id="dealLeaderName" type="text" readonly value="" style="width:75%;"/>
	<input id="dealLeaderLoginName" type="hidden" readonly value="" style="width:80%;" />
	<input id="dealLeaderDeptId" type="hidden" readonly value="" style="width:80%;" />
