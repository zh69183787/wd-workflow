...
<html>
	<head>
		...
		function btnClose(num) {
	        ...
	        if(window.opener){
	        	$("#normativeMemoTable tr:eq(<%=fileGroup%>) td:eq(4) input[name=normativeAttach]",window.opener.document).val($("#fileGroupId").val());
				$("#normativeMemoTable tr:eq(<%=fileGroup%>) td:eq(4) input[name=normativeAttachCount]",window.opener.document).val($("#count").val());
				if($("#count").val() > 0){
                    $("#normativeMemoTable tr:eq(<%=fileGroup%>) td:eq(4) input[name=normativeAttachCount]",window.opener.document).next().html("<img alt='附件' src='<%=path%>/send/images/fj.gif'/>");
                }else{
                    $("#normativeMemoTable tr:eq(<%=fileGroup%>) td:eq(4) input[name=normativeAttach]",window.opener.document).val("");
                    $("#normativeMemoTable tr:eq(<%=fileGroup%>) td:eq(4) input[name=normativeAttachCount]",window.opener.document).next().html("");
                }
						...
						<td style="line-height: 15px">
							...
							<%
								...
								if (StringUtil.isNull(procType)|| !procType.equals("view")) {
									%>
							<a href="<%=request.getContextPath()%>/attach/deleteFilesJsp.action?deleteData=<%=file.getAttachFile().getId()%>&fileGroup=<%=fileGroup%>&fileGroupId=<%=fileGroupId%>&fileGroupName=<%=fileGroupName%>&targetType=<%=targetType%>&fileCntObjId=<%=fileCntObjId%>&attachMemo1=<%=attachMemo1%>&approve=<%=approve%>&listType=<%=listType%>"
