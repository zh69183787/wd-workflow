...
try {
	...
	properties.load(fis);
	previewUrl = properties.getProperty("previewUrl");
    specialDdUrl = properties.getProperty("specialDdUrl");
	fis.close();
} catch (Exception e) {
			...
			<s:form theme="simple" action="upFileJsp" enctype="multipart/form-data" method="post" namespace="/attach">
				...
				<table width="100%" cellpadding="2" cellspacing="0">
					...
					<tr>
						...
						<td align='center' nowrap="nowrap" width='10%' >
							<font style="font-family: 黑体;"><b>预览</b></font>
						</td>
					...
					<tr>
						<td style="line-height: 15px">
							...
							<% if((file.getUploadDate()!=null && file.getUploadDate().compareTo("2014-01-14 00:00:00")<0)||"G010060005132510".equals((String)session.getAttribute("login_name"))) {%>
                            ...
                            <a href="<%=specialDdUrl%>?fileId=<%=file.getAttachFile().getId()%>" target="_blank"
                               style="display: inline;"><%=file.getFileAllName()%></a>&nbsp;
							<%}else{ %>
								<%=file.getFileAllName()%>&nbsp;
							<%} %>
							<%
								if (StringUtil.isNull(procType)|| !procType.equals("view")) {
									if (("G010060005132510".equals((String)session.getAttribute("login_name"))) || (userName.equals(file.getUploader())&& (file.getUploadDate().startsWith(new Date(System.currentTimeMillis()).toString())))) { //不可操作模式
							%>
						...
						<%-- 在线预览 --%>
						<td style="word-break:break-all">
							<a href="<%=previewUrl %>?fileId=<%=file.getAttachFile().getId()%>" target="_blank">
							<img src="<%=request.getContextPath()%>/send/images/find.gif" />
							</a>
							&nbsp;
						</td>
