...
<head>
	...
	function getDwSerial(){
		$.post(
				'<%=path%>/receive-redvUtil/getSerial.action?random='+Math.random(),
				{
					"prefix":	$("#zleader").val(),
					"postfix": 	$("#xdept").val()
				},
	...
	function selDwSerial(){
		$.post(
				'<%=path%>/receive-redvUtil/selSerial.action?random='+Math.random(),
				{
					"prefix":	$("#zleader").val(),
					"postfix": 	$("#xdept").val()
				},
				function(obj, textStatus, jqXHR){
					...
					if(obj !=null && obj.length>0){
						for(var i=0;i<obj.length;i++){
							...
							'<input type="radio" name="dwSerialRadio" id="dwSerialRadio"'+ 
							' value="'+i+'"/></td>'+
							'<td>'+obj[i]+
							'<input type="hidden" name="dwSerials" value="'+obj[i]+'">'+
							'</td></tr>';
						}
						$("#dwSerialTab").html(temp);
					}else{
						$("#dwSerialTab").html("");
					}
					$( "#dwSerial_zone" ).dialog( "open" );
				},
				"json"
			).error(function() { alert("服务器连接失败，请稍后再试!"); });
	}
	...
	function updateSerial(){
		$.post(
				'<%=path%>/receive-redvUtil/updateSerial.action?random='+Math.random(),
				{
					"prefix":	$("#zleader").val(),
					"postfix": 	$("#xdept").val(),
					"serialno": 	$("#zdept").val(),
				...
				function(obj, textStatus, jqXHR){
					if(obj == "1"){
						alert("保存号码成功！");
						$("#deptSerial").html($("#zleader").val()+"["+$("#xdept").val()+"]"+$("#zdept").val()+"号");
	...
	$(function(){
		...
		$("#dwSerialGet").click(function(){
			getDwSerial();
		});
		
		$("#dwSerialSel").click(function(){
			selDwSerial();
		});
		
		$("#dwSerialDelay").click(function(){
			$("#zdept").val("");
		...
		$("#dwSerialAbandon").click(function(){
			if(confirm('您确定废除该呈批件并不进行编号？\n该按钮点击提交后将无法重新编号！\n如需要暂缓编号请选择“缓”按钮。')){
				$("#zdept").val("0");
				updateSerial();
			}
		});
		
		$("#dwSerialSelSubmit").on("click", function(){
			$("#zdept").val($("input[name=dwSerialRadio]:checked").parent("td").next("td").find("input[name=dwSerials]").val());
			$( "#dwSerial_zone" ).dialog( "close" );
		});
		...
		$("#todo_handle").on("click",function(){
			if($("#zdept").length > 0 && $("#zdept").val()==""){
				alert("请选择部门编号！");
			}else if($("#zdept").length > 0 && $("#zdept").val()!=""){
				if(confirm('您确定提交部门编号?')){
					updateSerial();
				}
			}else{
				$("#handle_zone").dialog("open");
...
<%}else if(SimulateSubStepConstants.STEPNAME_SIMULATE_SUGGEST.equals(steplabel) ||
		RedvMainStepConstants.STEPNAME_RECORD.equals(steplabel)){%>
	<s:include value="%{#request.getContextPath()}/receive/process/redv/record.jsp"></s:include>
...
<%}else if(FinishSubStepConstants.STEPNAME_FINISH.equals(steplabel)){%>
	...
	<s:include value="%{#request.getContextPath()}/receive/process/redv/dwSerial.jsp"></s:include>
<%} %>
<!--  -->
	<div class="f_bg">
        ...
        <div class="gray_bg">
        	<div class="gray_bg2">
            	<div class="w_bg">
                	<div class="Bottom">
                    	<div class="Top">
                        	<h1 class="t_c"><b style="display:inline;"><s:property value="params.userInfo.test"/>上海申通地铁集团有限公司</b><br><b>呈批文件单</b></h1>
                        	<s:if test='vo.flag=="0" && (vo.docLevel== "加急"||  vo.docLevel == "紧急")'>
								...
								<table id="form_detail_zone" width="100%" border="0" cellspacing="0" cellpadding="0" class="table_2">
									...
									<tr class="content7">
	                            	   ...
	                            	   <s:property value="vo.submitDate"/>     
	                            	    ...
	                            	    <td class="b0" colspan="3" id="deptSerial">
										<s:if test='vo.zdept==null && "办结"==#parameters.steplabel[0]'>
											<s:property value="vo.zleader"/>
											<input value="<s:property value="vo.zleader"/>" type="hidden" name="zleader" id="zleader"/>
											[ <select id="xdept" name="xdept">
	                            	     	<%
											...
											<%} %>
											 ...
											 &nbsp;
											 <input type="text" class="input_none" id="zdept" name="zdept" readonly="readonly"/> <font class="redMark">*&nbsp;</font>
		                            	  	<br>
		                            	    &nbsp;
		                            	    <input type="button" id="dwSerialGet" value="取"/>
		                            	    &nbsp;
		                            	    <input type="button" id="dwSerialSel" value="选"/>
		                            	     &nbsp;
		                            	    <input type="button" id="dwSerialDelay" value="缓"/>
		                            	    &nbsp;
		                            	    <input type="button" id="dwSerialAbandon" value="废"/>
                                  	...
                                  	<tr class="content7">
	                                    ...
	                                    <td colspan="11">
	                                    	<input type="hidden" name="attach" id="attach" value="<s:property value="vo.attach"/>"/>
										  	...
										  	src="<%=path %>/attach/loadFileList.action?fileGroup=attach&fileGroupName=attachGroup&fileGroupId=<s:property value="vo.attach"/>&userName=<s:property value="#session.user_name"/>&loginName=<s:property value="#session.login_name"/>&procType=view&targetType=frame&type=1"></iframe>
										</td>
	                          	    </tr>
									...
									<tr class="content7">
										<td class="lableTd t_v ">审核领导批示：</td>
										<td colspan="11">
									...
									<tr class="content7">
										<td class="lableTd t_v ">会稿部门意见：</td>
										<td colspan="11">
									...
									<tr class="content7">
										...
										<table width="100%" cellspacing="0" cellpadding="0" border="0" class="table_suggest">
											...
											<s:param name="pname" value="params.processParam['pname']"></s:param>
											<s:param name="pincident" value="params.processParam['pincident']"></s:param>
											<s:param name="type">ReDirective_notice</s:param>
											</s:action>
											</td>
											</tr>
										</table>
