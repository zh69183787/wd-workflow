...
<html>
  <head>
	...
	function openForm(pname,pincident,cname,cincident,steplabel,taskid){
			var url = "<%=path %>/receive-redvMain/forward.action?"
  ...
  <body>
	...
	<%
		...
		List<Map<String,Object>> list = ProcessUtil.generateRedvQueryList();
	...
	//System.out.println(list.size());
		...
		out.println("<tr class='list-title'><td>主流程</td><td>主流程ID</td><td>呈报部门</td><td>呈报日期</td><td>部门编号</td><td>标题</td><td>状态</td><td>查看</td><td>监控</td></tr>");
		
		if(list!=null && list.size()>0){
		for(int i=0;i<list.size();i++){
			Map<String,Object> map = list.get(i);
			String pname = map.get("activeid").toString();
			String pincident = map.get("PROCESSINSTANCEID").toString();
			String cname = map.get("activeid").toString();
			String cincident = map.get("PROCESSINSTANCEID").toString();
			String steplabel = "";
			...
			out.print("<td>"+StringUtil.getNotNullValueString(map.get("SUBMITDEPT"))+"</td>");
			out.print("<td>"+StringUtil.getNotNullValueString(map.get("SUBMITDATE"))+"</td>");
			out.print("<td>"+StringUtil.getNotNullValueString(map.get("DEPTID"))+"</td>");
	...
	<a href="<%=path %>/receive/process/redv/list.jsp" target="_blank">查询列表</a>
	</div>
  </body>
</html>
