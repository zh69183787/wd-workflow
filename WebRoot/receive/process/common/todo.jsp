...
function print(){
	...
	var url =  "";
	if(pname=="新收文流程"){
		url = "<%=path %>/receive-recvMain/print.action?"
			+"pname="+encodeURI(pname)+"&"
			+"pincident="+pincident+"&"
			+"rand="+Math.random();
	}else if(pname=="新收呈批件"){
		url = "<%=path %>/receive-redvMain/print.action?"
			+"pname="+encodeURI(pname)+"&"
			+"pincident="+pincident+"&"
			+"rand="+Math.random();
	}
...
function prefill(flag){
	var type = $("input[name='pname']:hidden").val();
	var url  = "";
	if(type=="新收文流程"){
		url = '<%=path%>/receive-recvUtil/prefill.action?random='+Math.random();
	}else if(type=="新收呈批件"){
		url = '<%=path%>/receive-redvUtil/prefill.action?random='+Math.random();
	}
	$.post(
			url,
			{
				"id": 	'<s:property value="vo.id"/>',
				"flag": 	flag
			},
