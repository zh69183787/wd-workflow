...
<%@page import="com.wonders.receive.workflow.process.recv.constants.*"%>
...
$(function(){
	$("#handleSubmit").click(function(){
		...
		if(confirm("确认提交吗？")){
			...
			<%if(SimulateSubStepConstants.STEPNAME_SIMULATE_SUGGEST.equals(steplabel)) {%>
			options.url = "<%=path %>/receive-simulateSub/suggest.action";
			<%}else if(RecvMainStepConstants.STEPNAME_RECORD.equals(steplabel)){%>
			options.url = "<%=path %>/receive-recvMain/record.action";
			<%}%>
				...
				<tr>
	  			<td colspan=2>
			  		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		              	...
		              	<tr>
		              		<td class="td_1">&nbsp;</td>
		                	<td><textarea id="suggest" name="suggest" rows="5"><%if(RecvMainStepConstants.STEPNAME_RECORD.equals(steplabel)){%>备案完成。<%} %></textarea></td>
		              	</tr>
	              	</table>
           		</td>
           	</tr>
